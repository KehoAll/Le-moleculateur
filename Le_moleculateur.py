...
def parse_sheet_name(sheet_raw):
    if sheet_raw == "":
        return 0
    return int(sheet_raw) if sheet_raw.isdigit() else sheet_raw


def parse_precursors(precs_str):
    if not precs_str:
        print("ERREUR : Aucun precurseur fourni.")
        sys.exit(1)
    return precs_str.split()


def normalize_purete_list(purete_str, nb_init_prec):
    if purete_str:
...
def build_output_path(folder_name, filename_base):
...
def format_removed_spectators(removed_list):
    if not removed_list:
        return ""
    return ", ".join(f"{name} x{k}" for name, k in removed_list)


def add_common_args(parser, last_config, show_examples=True):
    precs_help = "Liste des precurseurs separes par des espaces."
    if show_examples:
        precs_help += " (ex: CaO Ga2O3)."
    parser.add_argument(
        "--Precs",
    ...
    parser.add_argument(
        ...
        required=True,
        default=last_config.get("path", os.path.join(os.getcwd(), "Atomic weights.xlsx")),
        widget="FileChooser",
    )
...
def add_solid_args(parser, last_config):
    parser.add_argument(
        "--Prod",
    ...
    # Element de normalisation optionnel
    parser.add_argument(
        "--PivotEl",
    ...
    parser.add_argument(
        ...
        help=(
            "Liste d'especes gazeuses supplementaires separees par des espaces ou des virgules "
            "(ex: CO NO N2 H2S)."
        ),
...
def add_solution_args(parser, last_config):
    parser.add_argument(
        "--Solute",
        metavar="Formule cible (obligatoire)",
        help="Formule brute cible en solution.",
        required=True,
        default=last_config.get("Solute", ""),
        widget="TextField",
    )
    parser.add_argument(
        "--C",
        metavar="Concentration (mol/L)",
        help="Concentration en mol/L.",
        required=False,
        default=last_config.get("C", ""),
        type=str,
        widget="TextField",
    )
    parser.add_argument(
        ...
        help="Volume en mL.",
        required=False,
        default=last_config.get("V", ""),
        type=str,
        widget="TextField",
    )
    parser.add_argument(
        "--Spectators",
        metavar="Ions spectateurs (optionnel)",
        help="Liste d'ions spectateurs separes par des espaces ou des virgules (ex: Cl NO3 SO4 AcO).",
        default=last_config.get("Spectators", ""),
        widget="TextField",
    )
    parser.add_argument(
        "--AutoSpectators",
        action="store_true",
        default=last_config.get("AutoSpectators", True),
        help="",
        gooey_options={"label": "Calcul automatique des ions spectateurs"},
    )

def run_solid(args, last_config, config_path):
    ...
    precursors = parse_precursors(precs_str)

    purete_list = normalize_purete_list(args.Purete.strip(), len(precursors))

    sheet_name = parse_sheet_name(sheet_raw)
    ...
    if len(purete_list) != nb_init_prec:
        purete_list = normalize_purete_list(args.Purete.strip(), nb_init_prec)
    ...
    for i, prec_name in enumerate(precursors):
        ...
        if i < nb_init_prec:
            ...
            if mass_prec_i >= 0:
                results_consumed.append(
                    (prec_name, mass_corr, dmass_corr, pcent, mass_corr - mass_prec_i, M_prec_i)
                )
            else:
    ...
    if pivot_el_user:
        if pivot_el_user not in final_dict or final_dict[pivot_el_user] <= 0.0:
            print(
                f"Avertissement : element de normalisation '{pivot_el_user}' absent ou nul dans la formule finale. "
    ...
    if denom > eps:
        ...
        print(
            f"Avertissement : impossible de normaliser sur {use_ref_el} (denominateur nul). "
            "Facteur d'echelle fixe a 1.0."
        )
        scale = 1.0
    ...
    candidate_path = build_output_path(folder_name, filename_base)
    ...
    for (pname, mass_corr, dmass_corr, pcent, delta_m, M_prec_i) in results_consumed:
        ...
        print(
            f" * {pname:<8s} : {mass_corr:.{decimals}f} +/- {dmass_corr:.{decimals}f} g  {purete_str_print}"
        )
    ...
    if total_mass < 1e-12 or total_moles < 1e-12:
        ...
        print(
            f"Total masse precurseurs : {total_mass:.{decimals}f} +/- {total_mass_uncert:.{decimals}f} g"
        )
        for (pname, m_corr, n_corr) in consumed_data:
            frac_mass = m_corr / total_mass
            frac_mole = n_corr / total_moles
            print(
                f" - {pname:<8s} : masse = {m_corr:.{decimals}f} g,  fraction massique = {frac_mass*100:.2f}%, "
            ...
            for (pname, mass_corr, dmass_corr, pcent, delta_m, M_prec_i) in results_consumed:
                writer.writerow(
                    [
                        "precurseur",
                        pname,
                        f"{mass_corr:.{decimals}f}",
                        f"{dmass_corr:.{decimals}f}",
                        f"{pcent:.2f}",
                        f"{delta_m:.{decimals}f}",
                        f"{M_prec_i:.6f}",
                    ]
                )
            for (pname, mass_rel, dmass_rel) in results_byproducts:
    ...
    save_last_config(config_path, payload)

def run_solution(args, last_config, config_path):
    solute_formula = args.Solute.strip()
    decimals = args.prec
    tol = float(args.tol)
    excel_path = args.path
    sheet_raw = str(args.sheet).strip()
    ...
    def parse_float_value(raw_value, label):
        ...
        try:
            ...
            raise ValueError(f"{label} invalide : {raw_value}") from ex

    if not solute_formula:
        print("ERREUR : Formule cible manquante.")
        sys.exit(1)

    try:
        concentration = parse_float_value(args.C, "Concentration")
        volume_ml = parse_float_value(args.V, "Volume")
    except ValueError as ex:
        print(f"ERREUR : {ex}")
        sys.exit(1)

    if concentration < 0 or volume_ml < 0:
        print("ERREUR : Concentration et volume doivent etre positifs.")
        sys.exit(1)
    ...
    sheet_name = parse_sheet_name(sheet_raw)
    dict_masses = extract_molar_masses_and_uncertainties(excel_path, sheet_name=sheet_name)
    solute_dict = parse_formula(solute_formula)
    validate_elements(solute_dict, dict_masses, "solute")
    M_solute, dM_solute = get_molar_mass_and_uncert(solute_dict, dict_masses)

    precursor_dicts = [parse_formula(p) for p in precursors]
    for prec_name, prec_dict in zip(precursors, precursor_dicts):
        validate_elements(prec_dict, dict_masses, f"precurseur {prec_name}")

    spectators_raw = args.Spectators.strip()
    spectators = parse_spectator_list(spectators_raw)
    auto_spectators_used = False
    if args.AutoSpectators and not spectators_raw:
        spectators = suggest_spectators(solute_dict, precursor_dicts)
        auto_spectators_used = True
    spectator_hits = {name: 0 for name, _ in spectators}

    active_dicts = []
    removed_by_prec = []
    for prec_name, prec_dict in zip(precursors, precursor_dicts):
        active_dict, removed = remove_spectators(prec_dict, spectators)
        active_dicts.append(active_dict)
        removed_by_prec.append(removed)
        for spec_name, k in removed:
            spectator_hits[spec_name] = spectator_hits.get(spec_name, 0) + k

        extra_elements = set(active_dict.keys()) - set(solute_dict.keys())
        if extra_elements:
            print(
                f"Avertissement : elements actifs {', '.join(sorted(extra_elements))} dans {prec_name} absents de la cible."
            )

    if spectators:
        missing_specs = [name for name, _ in spectators if spectator_hits.get(name, 0) <= 0]
        if missing_specs:
            print(
                "Avertissement : ions spectateurs non retires : "
                + ", ".join(missing_specs)
            )
    ...
    x_stoich, residuals, rank, singular_values = solve_stoichiometry(
        solute_dict, active_dicts, nonneg=args.nonneg
    )
    ...
    for i, prec_name in enumerate(precursors):
        ...
        if mass_prec_i >= 0:
            results_consumed.append(
                (i, prec_name, mass_corr, dmass_corr, pcent, mass_corr - mass_prec_i, M_prec_i, n_prec_i)
            )
        else:
            results_negative.append(
                (i, prec_name, abs(mass_corr), abs(dmass_corr), abs(n_prec_i), pcent, M_prec_i)
            )

    total_elements = {}
    partial_stoech = []
    for i, p_dict in enumerate(active_dicts):
        xi = x_stoich[i]
        sto_dict_i = {}
        for el, val_el in p_dict.items():
            contrib = xi * val_el
    ...
    sorted_by_stoich = sorted(solute_dict.items(), key=lambda x: x[1], reverse=True)
    default_ref_el, default_ref_val = sorted_by_stoich[0]

    denom = total_elements.get(default_ref_el, 0.0)
    if denom > eps:
        scale = default_ref_val / denom
    else:
        print(
            f"Avertissement : impossible de normaliser sur {default_ref_el} (denominateur nul). "
            "Facteur d'echelle fixe a 1.0."
        )
        scale = 1.0
    ...
    errors_found = False
    for e in solute_dict:
        target = solute_dict[e]
        found = scaled_elements.get(e, 0.0)
        if target != 0:
            diff_rel = abs(found - target) / target
            if diff_rel > (tol + eps):
    ...
    folder_name = solute_formula
    timestamp_str = run_timestamp.strftime("%Y_%m_%d_%H_%M")
    filename_base = f"{solute_formula}_solution_with_{'_'.join(precs_str.split())}"
    filename_base = f"{timestamp_str}_{filename_base}"
    ...
    if purete_list:
        print(f"Purete (%)         : {', '.join(f'{p:.2f}' for p in purete_list)}")
    if spectators:
        label = "Spectateurs (auto)" if auto_spectators_used else "Spectateurs"
        print(f"{label:<18}: {', '.join(name for name, _ in spectators)}")
    else:
        label = "Spectateurs (auto)" if auto_spectators_used else "Spectateurs"
        print(f"{label:<18}: aucun")
    print(f"Concentration      : {concentration} mol/L")
    print(f"Volume             : {volume_ml} mL")
    print(f"Moles cible (n)    : {n_target:.{decimals}f} mol")
    print(f"Tol. verification  : {tol}")
    print(f"Export CSV         : {'oui' if args.exportCSV else 'non'}")
    print(f"Contraintes >= 0   : {'oui' if args.nonneg else 'non'}\n")
    print(f"Solute cible       : {solute_formula}")
    print(f"Masse molaire du solute : {M_solute:.3f} +/- {dM_solute:.3f} g/mol\n")

    print("===== Precurseurs (a peser) =====\n")
    for (_, pname, mass_corr, dmass_corr, pcent, delta_m, M_prec_i, n_prec_i) in results_consumed:
        purete_str_print = ""
        if abs(pcent - 100.0) > 1e-9:
            purete_str_print = f"(purete={pcent:.1f}%, delta={delta_m:.4f} g)"
        print(
            f" * {pname:<8s} : {mass_corr:.{decimals}f} +/- {dmass_corr:.{decimals}f} g  "
            f"(n={n_prec_i:.{decimals}f} mol) {purete_str_print}"
        )

    print("\n===== Details solution =====\n")
    print(f"Moles totales de solute : {n_target:.{decimals}f} mol")
    ...
    consumed_data = []
    for (_, pname, mass_corr, dmass_corr, pcent, delta_m, M_prec_i, n_prec_i) in results_consumed:
        consumed_data.append((pname, mass_corr, n_prec_i))
        total_mass += max(mass_corr, 0)
        total_moles += max(n_prec_i, 0)
        total_mass_uncert_sq += dmass_corr ** 2
    ...
    if not spectators:
        ...
        any_removed = False
        for i, removed in enumerate(removed_by_prec):
            removed_str = format_removed_spectators(removed)
            if removed_str:
                print(f" - {precursors[i]} : {removed_str}")
                any_removed = True
        if not any_removed:
            print("  aucun")

    if results_negative:
        print("\n===== Precurseurs incompatibles (coeff negatif) =====\n")
        for (_, pname, mass_rel, dmass_rel, n_rel, pcent, M_prec_i) in results_negative:
            print(
                f" - {pname:<8s} : {mass_rel:.{decimals}f} +/- {dmass_rel:.{decimals}f} g  "
                f"(n={n_rel:.{decimals}f} mol)"
            )

    print("\n===== Stoechiometrie totale reconstituee (pour 1 mole du solute) =====")
    for el in sorted(total_elements.keys()):
        print(f"  Element {el} : {total_elements[el]:.3f}")
    ...
    print("\n===== Stoechiometrie apportee par chaque precurseur (pour 1 mole du solute) =====")
    for i, (prc_name, sto_dict) in enumerate(partial_stoech):
        print(f"  Precurseur : {prc_name} (x={x_stoich[i]:.3f})")
        for el in sorted(sto_dict.keys()):
            print(f"    {el} : {sto_dict[el]:.3f}")

    print(f"\nFormule brute reconstituee (normalisee sur {default_ref_el}) : {recon_formula}")
    ...
    if args.exportCSV:
        csv_path = os.path.splitext(candidate_path)[0] + ".csv"
        removed_strs = [format_removed_spectators(removed) for removed in removed_by_prec]
        with open(csv_path, "w", encoding="utf-8", newline="") as csv_file:
            writer = csv.writer(csv_file, delimiter=";")
            writer.writerow(
                [
            ...
            for (prec_index, pname, mass_corr, dmass_corr, pcent, delta_m, M_prec_i, n_prec_i) in results_consumed:
                writer.writerow(
                    [
                        "precurseur",
                        pname,
                        ...
                        f"{delta_m:.{decimals}f}",
                        f"{M_prec_i:.6f}",
                        f"{n_prec_i:.{decimals}f}",
                        removed_strs[prec_index] if prec_index < len(removed_strs) else "",
                    ]
                )
            for (prec_index, pname, mass_rel, dmass_rel, n_rel, pcent, M_prec_i) in results_negative:
                writer.writerow(
                    [
                        "incompatible",
                        pname,
    ...
    payload.update(
        {
            "mode": "Solution",
            "Solute": solute_formula,
            "C": concentration,
            "V": volume_ml,
            "Spectators": args.Spectators.strip(),
            "AutoSpectators": args.AutoSpectators,
            "Precs": precs_str,
            "Purete": args.Purete.strip(),
            "prec": decimals,
            "tol": tol,
...
def main():
    ...
    parser = GooeyParser(
        description=(
            "Calcule les masses de precurseurs (avec incertitudes), "
            "et permet d'ajuster la decomposition en CO2 / O2 / H2O / NO2 + autres gaz libres."
        )
    )

    subparsers = parser.add_subparsers(dest="mode")
    common_solid = GooeyParser(add_help=False)
    add_common_args(common_solid, last_config, show_examples=True)

    common_solution = GooeyParser(add_help=False)
    add_common_args(common_solution, last_config, show_examples=False)

    solid_parser = subparsers.add_parser("Solide", parents=[common_solid])
    add_solid_args(solid_parser, last_config)

    solution_parser = subparsers.add_parser("Solution", parents=[common_solution])
    add_solution_args(solution_parser, last_config)

    args = parser.parse_args()
    mode = getattr(args, "mode", None) or last_config.get("mode", "Solide")

    if mode == "Solution":
        run_solution(args, last_config, config_path)
    else:
        run_solid(args, last_config, config_path)
